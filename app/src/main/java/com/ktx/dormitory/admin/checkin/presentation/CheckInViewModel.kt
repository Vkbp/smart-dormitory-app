...
class CheckInViewModel @Inject constructor(
    ...
    override fun onEvent(event: CheckInUiEvent) {
        when (event) {
            ...
            CheckInUiEvent.ClearStatus -> updateState { it.copy(successMessage = null, errorMessage = null) }
        }
    }
    ...
    private fun search(cccd: String) {
        ...
        searchJob = viewModelScope.launch {
            try {
                ...
                Log.d("PERF_LOG", "API Search Request started for CCCD: $cccd")

                searchStudentUseCase(cccd)
                    .onSuccess { student ->
                        val endTime = System.currentTimeMillis()
                        Log.d("PERF_LOG", "API Search Response received in ${endTime - startTime}ms")
                        updateState { it.copy(studentInfo = student.toDomain()) }
                    }
                    .onFailure { error ->
                        val endTime = System.currentTimeMillis()
                        val errorMessage = error.toUserFriendlyMessage()
                        Log.w("PERF_LOG", "API Search Failed after ${endTime - startTime}ms: $errorMessage", error)
                        updateState { it.copy(errorMessage = errorMessage) }
                    }
            } catch (e: Exception) {
                if (e !is kotlinx.coroutines.CancellationException) {
                    Log.e("PERF_LOG", "Unexpected error in search: ${e.message}")
