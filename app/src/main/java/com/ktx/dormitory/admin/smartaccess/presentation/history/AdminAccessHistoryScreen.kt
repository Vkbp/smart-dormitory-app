...
fun AdminAccessLogItem(
    ...
    val location = log.gateName ?: log.gateId ?: log.buildingId ?: "Hệ thống"
        ...
        supportingContent = {
            Column {
                ...
                if (log.studentId != null) {
                    val studentInfo = if (log.studentName != null) {
                        "${log.studentName} - ${log.studentCode ?: ""}"
                    } else {
                        DataFormatter.formatId(log.studentId)
                    }
                    Text(
                        text = "Sinh viên: $studentInfo",
                        style = MaterialTheme.typography.bodySmall
                    )
                }
                val operatorDisplay = log.operatorName ?: DataFormatter.formatOperator(log.operatorId)
                Text(
                    text = "Thực hiện bởi: $operatorDisplay",
                    style = MaterialTheme.typography.bodySmall,
