...
class RoomRepositoryImpl @Inject constructor(
    ...
    override suspend fun cancelTransferRequest(id: Long): Result<Unit> {
        return try {
            val response = remoteDataSource.cancelTransferRequest(id)
            if (response.success) {
                Result.success(Unit)
            } else {
                Result.failure(Exception(response.message))
    ...
    override suspend fun getRoomUtilities(): Result<List<UtilityReading>> {
        return try {
            val response = remoteDataSource.getRoomUtilities()
            if (response.success && response.data != null) {
                Result.success(response.data.map { it.toDomain() })
            } else {
                Result.failure(Exception(response.message))
