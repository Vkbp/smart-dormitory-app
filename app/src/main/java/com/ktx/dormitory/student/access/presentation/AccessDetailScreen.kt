        ...
        Column(
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(20.dp)) {
                    ...
                    DetailRow(
                        label = "Địa điểm",
                        value = event.gateName ?: event.buildingName ?: if (event.gateId != null) DataFormatter.formatId(event.gateId) else "Thiết bị không xác định"
                    )
                    HorizontalDivider(modifier = Modifier.padding(vertical = 12.dp), thickness = 0.5.dp)
                    ...
                    if (event.method == "REMOTE_ADMIN" && !event.operatorName.isNullOrBlank()) {
                        HorizontalDivider(modifier = Modifier.padding(vertical = 12.dp), thickness = 0.5.dp)
                        DetailRow(
                            label = "Quản trị viên mở cửa",
                            value = event.operatorName
                        )
                    }
