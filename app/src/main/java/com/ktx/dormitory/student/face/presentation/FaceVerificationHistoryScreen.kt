    ...
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                Text(
                    text = attempt.gateName ?: if (attempt.gateDeviceId != null) "Nhận diện tại: ${DataFormatter.formatId(attempt.gateDeviceId)}" else "Cổng không xác định",
                    style = MaterialTheme.typography.bodyLarge,
                    fontWeight = FontWeight.Bold
                )
                Text(
                    text = DateTimeUtils.formatRelativeTime(attempt.attemptedAt) + " • Khuôn mặt (Face AI)",
                    style = MaterialTheme.typography.bodySmall,
                    color = Color.Gray
                )
                Text(
                    text = when (attempt.status.uppercase()) {
