...
*   **UC Lớn 2.2: Quản lý Phòng & Giường**
    *   *Chi tiết:* Tạo Phòng, Xem danh sách Phòng, Xem chi tiết Phòng, Cập nhật Phòng, Đổi trạng thái Phòng, Xóa Phòng, Tạo Giường thủ công, Tự động sinh Giường, Đổi trạng thái Giường, Xóa Giường.
*   **UC Lớn 2.3: Tra cứu & Phân tích Cơ sở vật chất**
    *   *Chi tiết:* Xem kết quả xếp phòng (SV), Xem thông tin phòng ở hiện tại (SV), Xem phòng còn trống (SV), Xem đếm ngược hạn xác nhận, Báo cáo phân tích phòng (Admin).
*   **UC Lớn 2.4: Quản trị Khóa thông minh (Smart Lock)**
    *   *Chi tiết:* Quản lý Mã PIN cửa phòng, Sinh PIN hàng loạt.

### 3. Phân hệ Dịch vụ lưu trú
*   **UC Lớn 3.1: Đăng ký Lưu trú & Xét duyệt**
    *   *Chi tiết:* Xem đợt đăng ký đang mở, Kiểm tra điều kiện đăng ký, Nộp đơn đăng ký nội trú, Tải lên minh chứng, Tra cứu trạng thái hồ sơ, Nộp lại minh chứng bị yêu cầu, Tạo và kích hoạt đợt đăng ký, Import danh sách SV đủ điều kiện, Xét duyệt hồ sơ đăng ký, Yêu cầu bổ sung hồ sơ.
*   **UC Lớn 3.2: Thực thi Lưu trú (Check-in/Check-out)**
    *   *Chi tiết:* Xác nhận thanh toán (Admin), Xem & Xác nhận Check-in, Nộp đơn trả phòng (SV), Duyệt đơn trả phòng (Admin).
*   **UC Lớn 3.3: Quản lý Biến động Lưu trú**
    *   *Chi tiết:* Nộp đơn gia hạn lưu trú, Duyệt đơn gia hạn, Nộp đơn xin chuyển phòng (SV), Duyệt đơn chuyển phòng (Admin), Dời phòng khẩn cấp (bảo trì).
...
### 5. Phân hệ An ninh & Thông báo
*   **UC Lớn 5.1: Kiểm soát Cổng ra vào thông minh**
    *   *Chi tiết:* Xác thực thẻ RFID tại cổng, Xác thực khuôn mặt tại cổng, Xác thực mã PIN cửa phòng, Lấy danh sách RFID hợp lệ, Báo lỗi phần cứng, Đồng bộ log offline IoT, Xem lịch sử ra vào (Admin), Mở cửa từ xa, Mở cửa khẩn cấp toàn hệ thống.
...
*   **UC Lớn 5.3: Quản trị Thiết bị IoT & Chính sách**
    *   *Chi tiết:* Quản lý Cổng/Thiết bị IoT, Cấu hình khung giờ RFID/Face, Cấu hình chính sách giờ giới nghiêm, Đồng bộ trạng thái IoT thủ công.
*   **UC Lớn 5.4: Quản trị Cư trú & Giờ giới nghiêm**
    *   *Chi tiết:* Xin phép về trễ (SV), Duyệt đơn về trễ (Admin).
*   **UC Lớn 5.5: Hệ thống Thông báo & Phản ánh**
    *   *Chi tiết:* Gửi thông báo hàng loạt, Nhận & Xem thông báo (SV/Admin), Báo cáo sự cố cơ sở vật chất.
...
*   **Các API / Chức năng được gộp:**
    ...
    *   Kiểm tra logic giờ giới nghiêm (UC 5.3)
    *   Lưu lịch sử vào/ra thành công (UC 5.1)
    *   Bắn thông báo vi phạm về trễ / Cảnh báo cạy cửa, lỗi phần cứng (UC 5.5)
...
*   **Các API / Chức năng được gộp:**
    ...
    *   Tự động sinh và lưu trữ Mã PIN cửa thông minh cho từng Phòng (UC 2.4)

---

## PHÂN TÍCH ĐỘ PHỦ NGHIỆP VỤ (API COVERAGE ANALYSIS)
Dựa trên danh sách 97 chức năng chi tiết và 6 Luồng Nghiệp vụ cốt lõi, dưới đây là bản đồ phân loại: Những API nghiệp vụ nào được "gom" vào 6 sơ đồ liên kết, và những API nghiệp vụ/CRUD nào đứng ngoài.
...
3.  **Thuộc Phân hệ An ninh & IoT (UC 5.x):** Đăng ký/Thu hồi Face ID, Gắn thẻ RFID (Flow 3, Flow 5); Push đồng bộ dữ liệu xuống IoT (Flow 3); Xác thực Face/RFID/PIN tại cổng, Kiểm tra giới nghiêm, Ghi lịch sử ra vào, Cảnh báo phần cứng (Flow 4).
...
2.  **Nghiệp vụ Báo cáo & Tra cứu (Read-only):** Xem Dashboard thống kê (UC 1.3), Báo cáo phân tích phòng (UC 2.3), Xem kết quả xếp phòng/đếm ngược (UC 2.3). Các API này thuần gọi lệnh `GET` + `Query` từ Database, không làm thay đổi trạng thái hệ thống.
...
4.  **Nghiệp vụ Nhánh rẽ/Phụ trợ:** Yêu cầu bổ sung hồ sơ, Nộp lại minh chứng (Đây chỉ là luồng phụ Alternative Flow của Flow 1); Xin phép/Duyệt đơn về trễ (UC 5.4 - logic của nó y hệt luồng xin chuyển phòng, không cần vẽ lặp lại).
5.  **Các API thuần CRUD và Quản lý (Sẽ được đại diện bằng 5 Sơ đồ Tuần tự CRUD mẫu tương ứng với 5 Phân hệ):**
    ...
    *   *Dịch vụ lưu trú:* Tạo và kích hoạt đợt đăng ký, Import danh sách SV đủ điều kiện, Xem đợt đăng ký đang mở, Tra cứu trạng thái hồ sơ, Nộp đơn gia hạn lưu trú, Duyệt đơn gia hạn, Dời phòng khẩn cấp.
