...
- **Tập trung Core Business:** Chỉ tập trung phân tích SÂU (vẽ sequence, activity, exception handling) cho các luồng phức tạp nhất:
  1. Thuật toán tự động xếp phòng (Housing Assignment).
  2. Xác thực Face Recognition (FaceNet) & Điều khiển cổng qua MQTT.
  3. Gạch nợ tự động (Automatic Payment) qua Webhook SePay, đảm bảo tính Idempotency.
  4. Giám sát thiết bị IoT và Cảnh báo sự cố thời gian thực.
