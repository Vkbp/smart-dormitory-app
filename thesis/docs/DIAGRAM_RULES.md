...
## 9. QUY TẮC ĐẶC THÙ CHO SƠ ĐỒ TUẦN TỰ (SEQUENCE DIAGRAM - CHƯƠNG 3)
1. **Kiến trúc N-Tier/ECB:** Bắt buộc thể hiện đủ các lớp: `Actor` -> `Boundary (UI)` -> `Controller` -> `Service` -> `Repository/Entity`.
2. **Đặt tên Lifeline:** Sử dụng format `:B_ClassName`, `:C_ClassName`, `:S_ClassName`, `:R_ClassName` để phân biệt vai trò.
3. **Độ chính xác Method:** Tên hàm, tham số và giá trị trả về phải khớp 100% với file `FEATURE_MAP.md` và mã nguồn (Java/TypeScript).
4. **Xử lý Ngoại lệ (Exception):** Phải có các khối `alt` để thể hiện luồng thất bại (Early Return) ngay khi validate dữ liệu không thành công.

## 10. QUY TẮC ĐẶC THÙ CHO SƠ ĐỒ HOẠT ĐỘNG (ACTIVITY DIAGRAM - CHƯƠNG 3)
1. **Hội tụ (Convergence):** Khác với Sequence Diagram, Activity Diagram phải đảm bảo mọi nhánh rẽ (Thành công/Thất bại) cuối cùng đều phải hội tụ về một điểm "Kết thúc" duy nhất để hoàn thành quy trình nghiệp vụ.
2. **Mô tả Action:** Sử dụng động từ + danh từ (VD: "Kiểm tra số dư", "Cập nhật trạng thái").
3. **Quy tắc Hình thoi:** Tuyệt đối không ghi chữ vào hình thoi. Ghi mô tả điều kiện ở khối chữ nhật ngay bên trên hình thoi.

## 11. DANH SÁCH LỖI TỬ HUYỆT CẦN TRÁNH (ANTIPATTERNS)
- **Thiếu mxGeometry:** Mọi mũi tên (`edge="1"`) thiếu thẻ con `<mxGeometry relative="1" as="geometry" />` sẽ bị tàng hình.
- **HTML Parsing Error:** Dùng `html=1` cho các label có dấu `< >` (như `<<include>>`) sẽ làm hỏng file XML. Luôn dùng `html=0`.
- **Comment Error:** Cấm dùng gạch ngang kép `--` trong comment XML.
